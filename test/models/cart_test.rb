...
class CartTest < ActiveSupport::TestCase
  def setup
    ...
    @productB = Product.create(name: "sticker", description: "a sticker from brandeis that is the perfect size for a phone", price: 50.0, category_id: (Category.all[0]).id, user_id: @userA.id, condition_id: Condition.all.first.id, quantity: 2)
    @productC = Product.create(name: "something else", description: "a sticker from brandeis that is the perfect size for a phone", price: 50, category_id: (Category.all[0]).id, user_id: (User.all[0]).id, quantity: 2)
