...
class ProductTest < ActiveSupport::TestCase
  def setup
    ...
    @ProductB = Product.new(name: "sticker", description: "a sticker from brandeis that is the perfect size for a phone", price: 50.0, category_id: (Category.all[0]).id, user_id: (User.all[0]).id, condition_id: Condition.all.first.id)
