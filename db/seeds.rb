...
20.times do
    Product.create(name: Faker::Commerce.product_name, description: (Faker::Lorem.paragraphs(number: 1)).join(" "), price: rand(100), user_id: User.all.ids[rand(0 .. 19)], status: [true, false].sample, quantity: rand(3), category_id: Category.all.ids[rand(0..7)], condition_id: Condition.all.ids[rand(0..3)]) 
end
